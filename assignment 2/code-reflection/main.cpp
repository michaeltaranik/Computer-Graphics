...
glm::vec3 trace_ray(const Ray &ray, int depth) {
  ...
  if (closest_hit.hit) {
    if (closest_hit.object->isMirror && depth < MAX_RECURSION) {
      glm::vec3 reflected_direction = glm::reflect( glm::normalize(ray.direction), glm::normalize(closest_hit.normal));
      Ray reflection_ray(closest_hit.intersection + closest_hit.normal * TOLERANCE, reflected_direction);
      color = trace_ray(reflection_ray, depth + 1);
    } else {
...
int main(int argc, const char *argv[]) {
  ...
  int width = 2*1024;  // width of the image
  int height = 2*768;  // height of the image
  ...
  for (int i = 0; i < width; i++)
    for (int j = 0; j < height; j++) {
      ...
      image.setPixel(i, j, glm::clamp(toneMapping(trace_ray(ray, 0)), glm::vec3(0.0), glm::vec3(1.0)));
    }
