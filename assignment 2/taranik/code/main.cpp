 ...
 private:
  ...
  Hit coneSurfaceIntersect(Ray localRay, glm::vec3 worldOrigin) {
    ...
    glm::vec3 worldIntersection =
        glm::vec3(transformationMatrix * glm::vec4(localIntersection, 1.0f));
    glm::vec3 worldNormal =
        glm::normalize(glm::vec3(normalMatrix * glm::vec4(localNormal, 0.0f)));
 ...
 public:
  ...
  Hit intersect(Ray ray) {
    ...
    glm::vec3 localDirection =
        glm::vec3(inverseTransformationMatrix * glm::vec4(ray.direction, 0.0f));
    ...
    if (baseHit.hit) {
      ...
      if (radius2 < 1.0f) {
        baseHit.intersection =
            glm::vec3(transformationMatrix * glm::vec4(localBasePoint, 1.0f));
        baseHit.normal =
            glm::vec3(normalMatrix * glm::vec4(baseHit.normal, 0.0f));
...
glm::vec3 PhongModel(glm::vec3 point, glm::vec3 normal,
  ...
  for (int light_num = 0; light_num < lights.size(); light_num++) {
    ...
    /*  ---- Exercise 3-----
                    ...
                    Include light attenuation due to the distance to the light
       source.
...
void sceneDefinition() {
  ...
  blue_specular.diffuse = glm::vec3(0.07f, 0.07f, 0.97f);
