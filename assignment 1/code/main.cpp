...
class Sphere : public Object {
 ...
 public:
  ...
  Sphere(float radius, glm::vec3 center, Material material, int rot)
      : radius(radius), center(center), rotation(rot) {
    this->material = material;
  }
...
glm::vec3 PhongModel(glm::vec3 point, glm::vec3 normal,
  ...
  for (Light *&light : lights) {
    ...
    float specular = glm::pow(
        glm::max(
            glm::dot(glm::normalize(view_direction + light_dir),  // half-vector
                     normal),
            0.0f),
        material.shininess);
    // float specular =
    //     glm::pow(glm::max(glm::dot(view_direction, reflection_direction),
    //     0.0f),
    //              material.shininess);
...
void sceneDefinition() {
  ...
  objects.push_back(
      new Sphere(1.0, glm::vec3(2.0, -2.0, 6.0), green_mat, 45));  // green
